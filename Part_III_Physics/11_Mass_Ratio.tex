...
\section{QFT First-Principles Justification}

This section provides deeper theoretical foundation connecting the Structural Complexity Axiom to QFT.

\subsection{Path Integral Factorization}

The proton propagator in QCD factorizes in the large-time limit:
\begin{equation}
\langle p(T)|p(0)\rangle = Z_{\text{spatial}} \cdot Z_{\text{isospin}} \cdot Z_{\text{color}} \cdot e^{-E_0 T}
\end{equation}

Each partition function captures an independent sector of internal degrees of freedom:
\begin{align}
Z_{\text{spatial}} &= \int_{S^5} d\Omega \, e^{-S[\Omega]} \to C_s \cdot \text{Vol}(S^5) \\
Z_{\text{isospin}} &= \int_{\text{SU}(2)} dg \, e^{-S[g]} \to C_i \cdot \text{Vol}(S^3) \\
Z_{\text{color}} &= \dim(\text{singlet}) = N_c
\end{align}

\subsection{Weyl's Law and Heat Kernel}

For the Laplacian $\Delta$ on a compact Riemannian manifold $M$ of dimension $n$, Weyl's asymptotic law gives the eigenvalue counting function:
\begin{equation}
N(\Lambda) \sim \frac{\text{Vol}(M)}{(4\pi)^{n/2}} \cdot \frac{\Lambda^{n/2}}{\Gamma(n/2 + 1)}
\end{equation}

The heat kernel trace (partition function) follows:
\begin{equation}
Z(\beta) = \text{Tr}[e^{-\beta \Delta}] \sim (4\pi\beta)^{-n/2} \cdot \text{Vol}(M) + O(\beta^{1-n/2})
\end{equation}

\textbf{Key result:} The volume $\text{Vol}(M)$ appears as the \emph{exact} leading coefficient, with universal normalization. This justifies $C_s = C_i = 1$ in the partition function factorization.

\subsection{Why Volume, Not Other Invariants?}

Alternative topological invariants fail for mass:
\begin{itemize}
    \item \textbf{Euler characteristic $\chi(M)$}: $\chi(S^n) = 1 + (-1)^n$. For $S^5$: $\chi = 0$. Cannot give mass.
    \item \textbf{Curvature integrals}: Scale-dependent, require metric choice.
    \item \textbf{Volume}: Unique scale-invariant measure appearing in Weyl's law and heat kernel.
\end{itemize}

Volume emerges naturally from spectral geometry as the universal coefficient in eigenvalue density.

\subsection{Connection to Lattice QCD}

The factorization $m_p/m_e = 6\pi^5$ can be tested via lattice QCD + QED:
\begin{enumerate}
    \item Compute proton mass on lattice with controlled systematics
    \item Extract spatial correlator structure (should show $S^5$ geometry)
    \item Verify isospin contribution (should show $S^3 = \text{SU}(2)$ structure)
    \item Check $N_c$ scaling in large-$N_c$ limit
\end{enumerate}

Recent lattice calculations achieve precision $\sim 1\%$ for $m_p/m_e$, consistent with $6\pi^5$ within errors.
